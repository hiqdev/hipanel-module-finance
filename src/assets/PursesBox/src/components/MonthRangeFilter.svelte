<script lang="ts">
  ...
  import { monthOptions, currentMonthKey, MONTH_NAMES } from '../data';
  ...
  function isEdge(k: string) { return k === value.from || k === value.to; }
  ...
  {#if open}
    <div class="filter-menu month-range-menu">
      <div class="filter-menu-hd">
        ...
        {#if isActive}
          <button type="button" class="link-action" onclick={() => { onChange({ from: null, to: null }); pickingFrom = true; }}>Clear</button>
        {/if}
      </div>
